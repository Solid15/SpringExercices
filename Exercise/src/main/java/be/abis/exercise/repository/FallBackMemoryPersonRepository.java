...
public class FallBackMemoryPersonRepository implements PersonRepository {
    ...
    private final ArrayList<Person> allPersons = new ArrayList<Person>();
    ...
    public FallBackMemoryPersonRepository() {
        ...
        allPersons.addAll(Arrays.asList(p1,p2,p3));
    }
    ...
    public Person findPerson(int id) {
        return allPersons.stream().filter(person -> person.getPersonId() == id).findFirst().orElse(null);
    }

    @Override
    public Person findPerson(String emailAddress, String passWord) {
        return allPersons.stream().filter(person ->
                (person.getEmailAddress().equals(emailAddress) && person.getPassword().equals(passWord)))
    ...
    //    allPersons.removeIf(person -> person.getPersonId() == id);
